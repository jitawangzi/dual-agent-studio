...
function isDiscussionReviewRole(role) {
    const r = String(role || '').trim().toLowerCase();
    return r === 'review' || r.startsWith('reviewer');
}

/**
 * Discussion CLI turns must fail-fast. A non-zero exit with partial text
 * (including a CONSENSUS / APPROVED blob) is still a failure.
 * Abort is not a failure — the caller already cancelled the turn.
 */
function interpretDiscussionAgentExit({
    aborted = false,
    timedOut = false,
    spawnError = null,
    exitCode = 0,
    ...
    const label = [role, provider].filter(Boolean).join(' ').trim() || 'Discussion agent';
    if (spawnError) {
        throw new Error(`DISCUSSION_AGENT_SPAWN_FAILED: ${label}: ${spawnError}`);
    }
    if (timedOut) {
        throw new Error(`DISCUSSION_AGENT_TIMEOUT: ${label} exceeded the watchdog.`);
    }
    if (exitCode !== 0) {
        throw new Error(`DISCUSSION_AGENT_EXECUTION_FAILED: ${label} CLI exited with code ${exitCode}.`);
    }
    return { aborted: false };
}

function buildDiscussionAgentCommand({ provider, model, reasoningEffort, sessionId, safeTmp, role } = {}) {
    ...
    const reviewRole = isDiscussionReviewRole(role);

    // Keep these flags aligned with engine/orchestrator-lib.ps1:
    // Build-CursorAgentArgs / Build-CodexExecArgs / Build-PiAgentArgs / Build-AgyPrintArgs.
    // Reviewer discussion turns use the same read-only / ask / restricted-tool set as the coding-loop reviewer.
    ...
    if (prov === 'antigravity' || prov === 'agy') {
        ...
        return `if (Get-Command agy, agy.exe -ErrorAction SilentlyContinue) { ${pipePrompt} | & agy ${agyArgs} --print } else { throw 'PROVIDER_UNAVAILABLE: Antigravity CLI (agy) is not found in PATH.' }`;
    }
    ...
    if (prov === 'codex') {
        ...
        const sandbox = reviewRole ? 'read-only' : 'workspace-write';
        return `if (-not (Get-Command codex, codex.exe, codex.cmd -ErrorAction SilentlyContinue)) { throw 'PROVIDER_UNAVAILABLE: OpenAI Codex CLI is not found in PATH.' }; ${pipePrompt} | & codex exec --skip-git-repo-check -a never -s ${sandbox}${modelArg}${effortArg} -`;
    }
    ...
    if (prov === 'pi') {
        ...
        const toolsArg = reviewRole ? ' --tools read,grep,find,ls' : '';
        return `if (-not (Get-Command pi, pi.exe, pi.cmd -ErrorAction SilentlyContinue)) { throw 'PROVIDER_UNAVAILABLE: Pi coding agent CLI is not found in PATH.' }; ${pipePrompt} | & pi -p${modelArg}${thinkArg}${toolsArg}`;
    }

    const copilotCheck = `if (-not (Get-Command copilot, copilot.cmd, copilot.ps1 -ErrorAction SilentlyContinue)) { throw 'PROVIDER_UNAVAILABLE: GitHub Copilot CLI is not found in PATH.' }; `;
    let cmd = `${copilotCheck}$copilotCmd = Get-Command copilot, copilot.cmd, copilot.ps1 | Select-Object -First 1; ${pipePrompt} | & $copilotCmd.Source -s --allow-all`;
